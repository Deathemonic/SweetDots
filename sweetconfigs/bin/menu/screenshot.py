...
class Capture:
    ...
    location_path, target_file = (
        utils.path_expander(
            screenshot_conf.get("directory", "$HOME/Pictures/Screenshots")
        ),
        f'Screenshot_{datetime.now():%Y-%m-%d-%H-%M-%S}_{"".join(choice(string.ascii_letters + string.digits) for _ in range(10))}.png',
    )
    ...
    def clipboard(self):
        if self.session == "wayland":
            with open(f"{self.location_path}/{self.target_file}", "rb") as output:
                run(
                    ["wl-copy", "-t", "image/png"], stdin=output
                ) if utils.check_installed("wl-copy") else logging.warning(
                    "wl-copy is not installed"
                )
        elif self.session == "x11":
            run(
                ["xclip", "-selection", "clipboard", "-t", "image/png", "-i", ""]
            ) if utils.check_installed("xclip") else logging.warning(
                "xclip is not installed"
            )

    def now(self):
        run(
            ["main", "-u", "-f", "png", f"{self.location_path}/{self.target_file}"]
        ) if self.session == "x11" else run(
            ["grim", f"{self.location_path}/{self.target_file}"]
        )
    ...
    def window(self):
        ...
        if utils.process_fetch("sway"):
            ...
            position = (
                ...
                .stdout.decode("utf-8")
                .strip()
                if utils.check_installed("jq")
                else exit(1)
            )

            run(["grim", "-g", position, f""])
        elif utils.process_fetch("Hyprland") or utils.process_fetch("hyprland"):
            active_window = run(["hyprctl", "activewindow"], capture_output=True).stdout
            position = (
                run(
                    ["awk", "NR==2 {print $2}"],
                    input=active_window,
            ...
            run(
                [
                    ...
                    "-g",
                    f"{position} {location}",
                    f"{self.location_path}/{self.target_file}",
                ]
            )
        else:
    ...
    def timer(self, time: int):
        ...
        run(
            ["grim", f"{self.location_path}/{self.target_file}"]
        ) if self.session == "wayland" else run(
...
def open_file() -> None:
    utils.notify(
        app="Clipboard", summary="Screenshot", body="Saved on Clipboard", urgent=0
    )
    run(
        ["xdg-open", f"{Capture.location_path}/{Capture.target_file}"]
    ) if utils.check_installed("xdg-open") else logging.warning(
        "cannot execute xdg-open"
    )
    if os.path.exists(f"{Capture.location_path}/{Capture.target_file}"):
        ...
        utils.notify(app="Screenshot", summary="Screenshot", body="Deleted", urgent=0)


def countdown(count: int) -> None:
    for sec in range(count, 0, -1):
        utils.notify(
            app="Countdown", summary="Screenshot", body=f"Capturing in: {sec}", urgent=0
        )
        sleep(1)
...
def get_selection(session: str):
    shot_conf, app = (
        Capture.screenshot_conf,
        utils.path_expander(utils.config.menu.get("app", "rofi")),
    )
    cmd = split(app)
    ...
    match chosen:
        ...
        case Choices.timer.value:
            Capture(session).timer(YAD().Scale(max=100, center=True, fixed=True, undecorated=True))  # type: ignore


def arguments():
    parser = ArgumentParser(description="a simple screenshot script")
    parser.add_argument(
        "-n", "--now", action="store_true", help="take a screenshot now"
    )
    parser.add_argument(
        "-w",
        "--window",
    ...
    parser.add_argument(
        "-a", "--area", action="store_true", help="take a screenshot of a specific area"
    )
    parser.add_argument(
        "-t", "--timer", action="store_true", help="set a timer to screenshot now"
    )
    parser.add_argument(
        "-m",
        "--menu",
...
def main() -> None:
    args, app = arguments(), utils.path_expander(utils.config.menu.get("app", "rofi"))
