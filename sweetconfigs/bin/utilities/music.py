...
def arguments():
    ...
    parser.add_argument(
        "-s", "--status", action="store_true", help="prints the status of the player"
    )
    parser.add_argument(
        "-t",
        "--text",
        action="store_true",
        help="prints the title and artist in a single string",
    )
    parser.add_argument(
        "-i", "--icon", action="store_true", help="prints the icon of the player status"
    )
    parser.add_argument(
        "-l",
        "--loop",
        action="store_true",
        help="loops the script and output a new string if theres a update",
    )
    parser.add_argument(
        "-n", "--space", action="store_true", help="don't add spacing on the text"
    )
...
def player_metadata(player, *other):
    ...
    if (
        "mpris:trackid" in player.props.metadata.keys()
    ...
    if args.status:
        ...
        if (
            player.props.status == "Stopped"
            or player.props.status == "Paused"
            and not args.space
        ):
...
def player_null_check(player_manager) -> bool:
    if len(player_manager.props.player_names):
        return True
    metadata = {
        "text": "Offline",
        "icon": "",
